...
<script type="text/javascript">
	 ...
	 function optDelete(name,isDisplay){
		 ...
		 //删除
		 $("#jvForm").attr("action","/back/brand/deletes.do?name="+name+"&isDisplay="+isDisplay);
...
<div class="box-positon">
	...
	<form class="ropt">
		<input class="add" type="button" value="添加" onclick="javascript:window.location.href='/back/brand/add.do'"/>
	</form>
	<div class="clear"></div>
</div>
<div class="body-box">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${pagination.list}" var="brand">
			<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
				...
				<td align="center"><img width="40" height="40" src="${brand.imgAllUrl}"/></td>
				<td align="center">${brand.description}</td>
				<td align="center">${brand.sort}</td>
				<td align="center"><c:if test="${brand.isDisplay==1}">是</c:if><c:if test="${brand.isDisplay==0}">不是</c:if></td>
				<td align="center">
				<a class="pn-opt" href="/back/brand/toEdit.do?id=${brand.id }">修改</a> | <a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;}" href="/back/brand/${brand.id}/delete.do?name=${brand.name}&isDisplay=${brand.isDisplay}">删除</a>
				</td>
			</tr>
		</c:forEach>
	</tbody>
