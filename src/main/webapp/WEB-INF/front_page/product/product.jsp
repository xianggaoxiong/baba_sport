	...
	<div class="r wr">
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			<ul class="uls filter">
				...
				<li><label>已选条件：</label>
				<p class="sel">
					<c:forEach items="${queryShow }" var="entity">
						<a href="javascript:void(0);">
						<em>${entity.key }</em>${entity.value }
				...
				<li><b>品牌：</b><p>
					...
					<c:forEach items="${brands }" var="brand">
						<a href="javascript:void(0);" title="${brand.name }" onclick="window.location.href='/front/product/list.shtml?brandId=${brand.id }&brandName=${brand.name}&typeId=${typeId }&typeName=${typeName }'">${brand.name }</a>
					</c:forEach>
				</p></li>
				</c:if>
				<li><b>价格：</b><p>
				...
				<li><b>类型：</b><p>
					...
					<c:forEach items="${types }" var="type">
						<a href="javascript:void(0);" title="${type.name }" onclick="window.location.href='/front/product/list.shtml?typeId=${type.id}&typeName=${type.name }&brandId=${brandId }&brandName=${brandName }'">${type.name }</a>
					</c:forEach>	
				</p></li>
				</c:if>
