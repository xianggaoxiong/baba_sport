			...
			<div class="ofc">
				...
				<form id="jvForm" action="profile.do" method="post">
					...
					<li>
						...
						<span class="word">
						<input type="radio" name="gender" <c:if test="${buyer.gender=='SECRECY' }">checked="checked"</c:if>/>保密
						<input type="radio" name="gender" <c:if test="${buyer.gender=='MAN' }">checked="checked"</c:if>/>男
						<input type="radio" name="gender" <c:if test="${buyer.gender=='WOMAN' }">checked="checked"</c:if>/>女
						</span>
					</li>
					<li>
